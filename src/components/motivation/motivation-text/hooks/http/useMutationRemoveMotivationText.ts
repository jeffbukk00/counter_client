...
const useMutationRemoveMotivationText = (
  ...
  const { mutate: mutateRemoveMotivationText } = useMutation({
    ...
    onSuccess: () => {
      // motivationText를 제거하는 비동기 요청이 성공 했을 시,
      // box가 담고 있는 모든 motivationText들의 id 리스트르 불러오는 비동기 요청의 캐시 업데이트.
    ...
    onError: () => {
      // motivationText를 제거하는 비동기 요청이 실패 했을 시,
      // 비동기 요청이 끝난 후, 로딩 상태에 대한 유저 피드백 종료.
      inactivate(boxId);
